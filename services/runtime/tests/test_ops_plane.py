...
def test_is_ops_l1_work_root() -> None:
    ...
    assert ops_plane.is_ops_l1_work_root("/data/ops-l1/cmteb-index/CovidRetrieval")
    assert ops_plane.is_ops_cmteb_work_root("/data/ops-l1/cmteb-index/CovidRetrieval")
    assert not ops_plane.is_ops_cmteb_work_root("/data/ops-l1/beir-index/fiqa")
...
def test_retrieval_url_routes_ops_when_configured(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ops_plane.settings, "ops_retrieval_pg_schema_zh", "retrieval_ops_zh"
    )
    ...
    zh = ops_plane.retrieval_database_url_for(
        work_root="/data/ops-l1/cmteb-index/CovidRetrieval"
    )
    assert product.endswith("/agent")
    assert ops.endswith("/bench")
    assert zh.endswith("/bench")
    assert (
        ops_plane.retrieval_pg_schema_for(work_root="/data/ops-l1/beir-index/fiqa")
        == "retrieval_ops"
    )
    assert (
        ops_plane.retrieval_pg_schema_for(
            work_root="/data/ops-l1/cmteb-index/CovidRetrieval"
        )
        == "retrieval_ops_zh"
    )
...
def test_settings_expose_ops_plane_fields() -> None:
    ...
    assert s.ops_retrieval_pg_schema_zh == "retrieval_ops_zh"
