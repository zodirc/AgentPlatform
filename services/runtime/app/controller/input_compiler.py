...
OUTLINE_EXPAND = (
    ...
    "用户要几章就写几章。"
    "默认写近处：每章一句章节作用，正在写的那章再写当前章段；不要写成小正文，也不要先排满后文章节。"
    "点明本章推进什么。纲用直说。"
    "短篇/单篇：纲可选；不要写成风格契约或世界法。"
    "长篇：先 propose_book_candidates 出作品候选，点选后再写开篇和第一章；"
    "前几章还在同一池子里往前；后面的海不要写进开篇。"
    "经典文学（literary）：第一章环境托人物，机构专名不要当开篇第一词；身世不进第一章。"
    "连载网文（web_serial）：只给题材时先写 2～3 本彼此不同的作品候选，点了再写开篇和第一章；"
    "第一章按已选的书写眼前这场。设定随事露头，勿提前兑卷末大高潮。"
    "长篇先写世界入口和当前阶段：读者跟谁进入，人物起初把异常当作什么；这一段开始时的位置、逐渐确认的事、阶段结束时的持久变化。远处只写阶段变化，不提前排满章节。近处章节各写一句作用：这一章之后，哪一件此前不成立的事成立了。章节顺序要有依赖。没有这一章的当前章段就不要写正文。"
    "这些写进大纲。交给正文的是事实，不是作用句、奖励或惩罚。"
    "不要另套一张「要/挡」总表，也不要先交全书规则。"
    "仅当用户要「简略/目录」时才可只写标题。"
    "批量扩章用 append；一章通常一场，长篇约三千到五千个可见字，不要为凑字另开冲突或灌无关说明。"
)
