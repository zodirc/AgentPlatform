...
5. **Verify**: fix `checks.new_issues` → prefer `related_tests` paths from the edit result → `read_lints` on touched paths if needed → **re-run the same Reproduce command** → claim done only when it passes (or explain why you cannot).
