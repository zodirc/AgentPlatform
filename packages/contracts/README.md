...
跨服务契约（无业务逻辑）。人类可读入口：[控制流导览 · 契约位置](../docs/tour/index.html#contracts)。
...
1. 事件 `type`：同时改 `events/types.json`、`events/payloads/`（若已有则改对应文件 + `_index.json`），并同步导览「契约位置」页。
2. 领域表：同时改 `ddl/phase0.sql` 或 `ddl/phase1_*.sql`。
3. 管理面 API（模型供应商等）：同时改 `openapi/public.yaml`、ADR-019。
4. 内部命令：同时改 `schemas/commands/*`。
5. 对外 API：同时改 `openapi/public.yaml`（Phase 0）与命令 schema（Phase 1）；并运行 `scripts/codegen.sh` 同步 `services/web` TS 类型（ADR-018）。
6. Golden Turn：同时改 `eval/golden/`、`packages/contracts/eval/golden_turn.schema.json`。
