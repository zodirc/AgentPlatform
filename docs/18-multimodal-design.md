...
> **性质**：设计稿（待落地）；落地后把票状态写回本文，**勿另开 execution 平行文**。宜配套 ADR-020。  
> **产品目标（一期）**：对话中可上传**图片**与**文本文件**；Agent 能读准文本、在有 Vision 时看懂图片；**不拖慢** Turn 热路径。  
> **一期范围**：仅 `kind = text | image`。**不做** PDF / Office / 音视频（见 §1）。  
> **约束继承**：[13-rate-redlines.md](13-rate-redlines.md) 速率红线 R1–R5；[10-product-experience.md](10-product-experience.md) TTFB ≤ 300ms、首模型 token ≤ 800ms（P95）；[ADR-014](adr/014-turn-intake-over-intent-pipeline.md) Intake 确定性编译。  
...
| 端用户 session 归属 | docs/16 | attachment 归属 session；Turn 前校验 |
