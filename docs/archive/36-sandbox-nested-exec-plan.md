...
> **状态**：A + C 已落地（2026-07-29）· 选择链 **Landlock → bwrap → off**，进程内 sticky  
> **触发**：Agent 自测 CLI 时 `run_command` 恒 `exit 1`；根因是镜像内有 bwrap，但宿主机/嵌套容器 **禁止非特权 user namespace**，命令在进业务 argv 前失败；模型误判为「禁网」。  
> **关联**：[31](../topics/sandbox.md) · [32](32-execution-plane-and-local-runner.md) · [BWRAP-mental-model.md](../learn/mental/bwrap.md) · 代码 `tools/core/sandbox.py` · `landlock_fs.py` · `shell.py`  
> **约束**：不改 Agent loop / 工具名 / 审批状态机；速率服从 [13](../core/13-rate-redlines.md)；默认仍 **护盘不禁网**。

---

## 0. 一句话

**runtime 继续跑在 Docker 里；OS 级 FS 笼子能开则开，开不了就诚实降级并让命令可跑；优先 Landlock（不靠嵌套 userns），其次 bwrap；长期才考虑独立执行面。**
...
图统一放在 `docs/assets/sandbox/`（与 [`BWRAP-mental-model.md`](../learn/mental/bwrap.md) 共用，不另开重叠根目录）。
...
| **5** | **Landlock 原理与执行流程（中文）** | [`landlock-exec-flow-zh.png`](../assets/sandbox/landlock-exec-flow-zh.png) |
| **6** | **审批门 × Landlock 分层（中文详图）** | [`landlock-vs-approval-flow-zh.png`](../assets/sandbox/landlock-vs-approval-flow-zh.png) |
| **7** | **为何能生效：缝手套≠戴上≠内核拦** | [`landlock-why-it-works-zh.png`](../assets/sandbox/landlock-why-it-works-zh.png) |
...
#### Landlock 原理与执行流程（中文详图）

![Landlock 原理与执行流程](../assets/sandbox/landlock-exec-flow-zh.png)

**读图要点（手套模型）**：

1. **不换房间**：不新建 mount / user namespace，进程仍在原 Docker 视图里。  
2. **戴手套**：`landlock_restrict_self` 后，内核按规则集拦截越权路径访问；规则**不可撤销**，子进程继承。  
3. **规则心智**：`/` → 读+执行；`work_root` → 读写；临时文件走 `TMPDIR=work_root/.agent-tmp`（图中若写作 `_agent-tmp` 以代码为准）。  
4. **施加时机**：父进程 `fork` 后、`exec` 前的 `preexec_fn`（`argv` 不被改写成 `bwrap …`）。  
5. **Sticky**：`resolve_sandbox_backend` 在进程内只选一次；结果标 `sandbox=landlock|bwrap|off`。
...
| 层 | 回答的问题 | 变了吗 |
|----|------------|--------|
| Agent 交互 / 审批 | **允不允许跑这条命令？** | ❌ 不变（次数、文案、状态机不因 Landlock 增加） |
| Landlock / bwrap | **跑起来后能写哪些盘？** | ✅ 仅执行面；批准之后才戴手套 |

ABI / `create_ruleset` / `add_rule` / `restrict_self` 都在**泳道 C**，与「点批准」正交。

#### 为何 create_ruleset / restrict_self「能生效」？

![为何能生效](../assets/sandbox/landlock-why-it-works-zh.png)

一句话：**缝手套 ≠ 戴上 ≠ 内核拦。** 真正拦住越界写的是内核 LSM，不是函数名本身。

| 阶段 | 调用 | 含义 |
|------|------|------|
| **1. 缝手套** | `create_ruleset`、`add_rule(/)`、`add_rule(work_root)` | 在内核里准备规则集；**此时进程尚未被限制** |
| **2. 戴上** | `prctl(NO_NEW_PRIVS)`、`landlock_restrict_self` | 规则挂到**当前线程**；成功后内核开始 enforce，**不可撤销** |
| **3. 跑命令** | `fork` → 子进程 `preexec`（阶段 1+2）→ `exec` | 只给命令子进程戴手套；父 runtime 不被箍死；后代继承 domain |

**为何一定要 `preexec`？**  
`restrict_self` 限制的是调用它的线程。若在父进程（整个 Agent runtime）里戴，服务自己读配置/连库都会被写狱误伤。所以：父 `fork` → **仅子进程**戴手套 → 再 `exec` 成 `pytest`/`sh`。

**「一定」生效吗？——不，失败就诚实降级。**

| 情况 | 本项目行为 |
|------|------------|
| 内核太旧（如 4.18）→ `ENOSYS` | 探针失败，不选 landlock |
| LSM 未开 / `restrict_self` 失败 | 同上 → 试 bwrap，再不行 `off` |
| syscall 全部成功 | 之后 `open`/`write` 进内核时按规则放行或 `EACCES` |

代码落点：`landlock_fs.py`（syscall）· `sandbox.py`（探测 + sticky）· `shell.py`（`preexec_fn`）。

| | **bwrap** | **Landlock** |
|--|-----------|---------------------------|
| 做法 | 新命名空间 + 重挂载「房间」 | 内核限制进程能摸的路径（「手套」） |
| 嵌套 Docker | 常卡在 user namespace | **更贴「外层已是容器」** |
| 你们现状 | ✅ 已实现（不可用则降级） | ✅ 已实现（内核 ≥5.13 + LSM；不可用则降级） |
| 产品目标 | 相同：可写≈作品根；护盘不默认禁网 | 同左 |
| 进程内 | **首次 resolve 钉住**，不每次重选 | 同左 |
...
5. **Sticky**：自动探测只在进程内做一次，之后持续用同一后端（重启才重探）。
...
| **A4** | 可观测 | `sandbox_status` 暴露 `landlock_usable` / `bwrap_usable` / `sticky` | Ops/健康能看见降级 |
...
### 批次 C — Landlock 适配嵌套 Docker（P1 · ✅ 已落地）

对标文档 31 原设计与 Codex：**进程树路径限制，不依赖嵌套 userns**。

| ID | 项 | 做法 | 验收 |
|----|-----|------|------|
| **C0** | 内核探测 | `landlock_abi_version` + 子进程 `restrict_self` 探针 | CI / 单测；ABI ENOSYS → 跳过 |
| **C1** | 实现 | `landlock_fs.py` + `sandbox.py` 后端：RW≈work_root；`TMPDIR`→`.agent-tmp`；`preexec_fn` 施加 | 单测 mock + 内核可用时集成：界内写 OK、界外写失败 |
| **C2** | 策略链 | **Landlock → bwrap → off(degraded)**；**sticky**；每级探测失败降级并打点 | `sandbox_status.backend` 稳定 |
| **C3** | 文档 | 更新 31 / 36 / BWRAP 心智「后端选择」 | 面试/运维口径一致 |

**依赖**：runtime 宿主机内核 ≥ 5.13 且启用 Landlock；不满足则自动落到 bwrap / off（本机 RHEL 8 / 4.18 = ENOSYS，属预期）。
...
口诀：**先能跑 → 再能看见降级 → 再用 Landlock 把笼子找回来（能开则 sticky）→ 执行面留给真正的多租户硬需求。**
...
| 风险 | 缓解 |
|------|------|
| 降级期缺 FS 笼子 | 文档诚实；多租户场景依赖内核升级开 Landlock；敏感环境可 fail-closed（拒绝 exec）作产品二选一 |
| Landlock 内核不够 | 探测失败自动跳过；行为同 A |
| 开 userns 的安全争议 | B 仅可选；默认不依赖 |
| 模型继续甩锅 | A3 + 评测金样「stderr 含 namespace 时应总结为沙箱降级/不可用」 |

回滚：`TOOL_SANDBOX=off|bwrap` break-glass；或回退镜像。
...
3. 内核支持 Landlock 的部署上，越界写盘被挡；否则诚实 `off` / `bwrap`。  
4. Agent 自测临时 API 有一条不污染聊天记录的路径（前缀或 D1）— D 仍待做。

---

## 7. 决策待确认（落地前勾选）

- [x] **是否投入 C（Landlock）** — ✅ 已落地为优先后端  
...
| 文档 | 用途 |
|------|------|
| [BWRAP-mental-model.md](../learn/mental/bwrap.md) | 大白话心智 |
| [31](../topics/sandbox.md) | 威胁与 SB 票 |
| [32](32-execution-plane-and-local-runner.md) | 执行面长期选项 |
| [03](../core/03-docker-runtime.md) | 部署与排障旋钮 |
