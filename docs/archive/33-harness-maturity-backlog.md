...
关联：[12](../core/12-model-harness.md)（Harness 总纲 · §5.1）· [13](../core/13-rate-redlines.md)（R1–R5）· [06](../core/06-tools-and-context.md) · [20](../learn/context-compaction-walkthrough.md) · [08](../core/08-event-projection-pipeline.md) · [11](../core/11-eval-and-golden-turns.md) · [15](../topics/rag-and-sources.md) · [14](../topics/writing/quality.md) · [24](../topics/writing/token-economy.md)（WT5）· [28](../topics/proof-gate-and-ux-signals.md) · [30](30-quality-and-agility.md)。
...
与 [30](30-quality-and-agility.md)、[12](../core/12-model-harness.md) 一致，本模块所有票必须同时满足：
...
2. **不影响交互速率**（服从 [13](../core/13-rate-redlines.md) R1–R5）  
...
结论与 [12](../core/12-model-harness.md) 一致：**Loop 形状已对；Harness 厚度与可证明性已由本文 HM 票加厚到核心落地**。
...
|---|------------------|------|------|------------------|
| Q2 | 短期记忆如何实现？ | 模型窗 = 组装后的 messages；多层压缩链；**HM1** 软阈值异步预压缩 | **已覆盖** | 机制见 [20](../learn/context-compaction-walkthrough.md) |
...
| Q7 | 已总结后下一轮：增量还是全量重算？ | **HM3**：`next = merge(prev, delta)`；全量仅 `/compact` / 熔断；单测钉边界 | **已覆盖** | 见 [20](../learn/context-compaction-walkthrough.md) |
...
| Q11 | 如何降低工具过多带来的 Token？ | Profile / Plan / late ToolScope；description hygiene；稳定前缀 cache | **已覆盖** | 面试口述见 [21](../learn/agent-system-qa.md) Q12 §5；**有意不做**预循环 LLM 选工具 |
...
| — | Harness / 规范工程 | [12](../core/12-model-harness.md) 六面；golden；ops 评测台 + 检索/信封 | **已覆盖（核心）** | 细项持续加厚 |
...
| **我们现在** | **HM1 已落地**：软阈值异步写缓存；硬阈值优先缓存 / 确定性增量摘要；同步 compact LLM 默认关（见 [20](../learn/context-compaction-walkthrough.md)） |
...
| **我们现在** | **HM3**：`merge(prev, delta)`；全量仅 `/compact`；单测钉 delta 边界（[20](../learn/context-compaction-walkthrough.md)） |
...
| **HM5** | 检索三层审计 → **Ops 观测台看前台用户 Turn** | **P0** | ✅ | **Q19** · [29](../topics/ops-eval-console.md) §6 |
...
- 单测：`test_hm_context_maturity.py`。文档：[20](../learn/context-compaction-walkthrough.md)。
...
- 文档：回写 [20](../learn/context-compaction-walkthrough.md) 顺序说明。
...
**问题**  
运行时已有 structured / LLM 摘要与 `/compact` 全量重算，但缺少明确产品契约：**续轮默认增量合并，禁止无条件全量重算**。压缩质量与「变短≈压缩」体感（[12](../core/12-model-harness.md) §5.1.2）缺少可回归钉子。
...
**验收**  
- 契约字段或内部结构文档化（可进 [06](../core/06-tools-and-context.md) / [20](../learn/context-compaction-walkthrough.md)）。  
...
- 不进 SSE 热路径；不进默认投影体积。  
- Ops / 评测台只读回放（见 [29](../topics/ops-eval-console.md)）。
...
**产品落点（先钉死）**  
主交付面是 **Ops 内部观测台**（[29](../topics/ops-eval-console.md) §6），**不是**前台 Web Agent / Writing 侧栏：
...
3. **Ops API + 页（主交付）**：见 [29](../topics/ops-eval-console.md) §6；与评测套件并列。  
4. **前台**：默认不改交互。  
...
- **非必过**：前台侧栏审计折叠。  
- 回写 [15](../topics/rag-and-sources.md) §3.3、[29](../topics/ops-eval-console.md) §6。

**触点（预期）**  
`retrieval/*` · `search_*` · `agent_engine` · contracts · `services/api/.../ops/` · Ops Web · [15](../topics/rag-and-sources.md) · [29](../topics/ops-eval-console.md)。
...
**问题**  
工具过多时 schema/描述占用窗口；写作/agent 的 prompt cache 易因「稳定前缀与易变 messages 未分家」整段 miss（[12](../core/12-model-harness.md) §5.1.1 · [24](../topics/writing/token-economy.md) WT5）。
...
- 遵守「长期记忆按需」（[06](../core/06-tools-and-context.md)）。
...
| 做法 | 常被当作「标准答法」的问题 | 原因 |
|------|---------------------------|------|
| 预循环 LLM 意图路由选工具 / 选是否检索 | Q9 / Q11 | R2；热路径多一整轮模型；与 ADR-014 / [13](../core/13-rate-redlines.md) 否决一致 |
...
| 改 while / 恢复 pipeline / 查询路径建索引 | （编排惯性） | 交互与速率红线；索引纪律见 [15](../topics/rag-and-sources.md) A9 |
...
与 [12](../core/12-model-harness.md) §5.1 的关系：  
- §5.1.1 WT5 ↔ **HM6**  
- §5.1.2 变短≈压缩 / 压缩质量 ↔ **HM1 + HM3**  
- §5.1.3 Proof 延迟盯梢 ↔ 既有 [28](../topics/proof-gate-and-ux-signals.md) + 本模块 golden；HM1 验收须带延迟对照  
...
2. [20](../learn/context-compaction-walkthrough.md) / [06](../core/06-tools-and-context.md) 已反映增量摘要与预压缩顺序；  
...
本仓（[15](../topics/rag-and-sources.md)，生产裁判栈）：
...
权威细节仍以 [15](../topics/rag-and-sources.md)、[20](../learn/context-compaction-walkthrough.md)、[14](../topics/writing/quality.md) 为准；本节只定对照结论与票映射。
