...
> 产品口径「变短≈压缩」、禁止无压缩暗削：见 [`12` §5.1.2](../README.md)。  
> **心智模型 + 流程图（大白话）**：先读 [`CONTEXT-mental-model.md`](mental/context.md)；本文偏逐步数字演练。
...
| 输出预留 | 30_000（128K→30K；随窗口等比例） | 给模型输出留空 |
...
> HM 细节见 [`33`](../archive/33-harness-maturity-backlog.md)。
