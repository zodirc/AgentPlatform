...
- 证据：`thinking.txt`（3.2 末班车轮；3.4 之后同题 live）；`eval/recordings/opening_cards/`（3.3 live）
...
- 用户点名题材：都市修真 live 3.3 见 `eval/recordings/opening_cards/`；3.4 同题多轮见仓库根 `thinking.txt`（未另存 json）。校园悬疑、历史架空未跑。
...
病不在都市修真，在把叙事当问答。选书已换成「交正文第一段」，3.2 那种简介换皮交不了卷。3.4 说明：卡上的「发生」会被读成短篇铰链；再用改提问去对齐热门开局会盖章。不要把场上的发生做成禁形或店招课；也不要在选书卡上模拟第一章。点选后的成章仍可能把第一章收成得到/发现，那是另一份范围。
