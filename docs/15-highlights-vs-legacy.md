...
- 可用 hybrid / 本地 embedding；也有 hash 降级路径，保证无重模型时仍可跑通；  
- retrieval 可作为 compose profile 增强，而不是强制所有环境装齐 GPU 依赖；  
- 「向量索引可用」≠「每轮强制 RAG」。
...
**一句话体感**：架构形状对了，自用已过门槛；下一阶段是继续加厚 Harness 与交付诚实性，而不是再造一张大图。
...
| 输入类型 | 处理 | 示例 |
|----------|------|------|
| 纯文本 | 进入 user content | 「请改第二节」 |
| Slash 命令 | 确定性解析，非 LLM | `/help`、`/compact`、`/version` |
| `@path` 引用 | 文件指针块；预算内可预读，超时降级为纯指针 | `@sections/02.md` |
...
| `search_sources` | retrieval | 文档/资料检索 |
...
| `delegate` | delegation | 派生子 agent |
| `update_plan` | planning | 更新计划投影 |
...
多 provider failover、GatewayRegistry+pg_notify、动态裁剪工具表、JSON mode 等。
...
| 能力 | 说明 |
|------|------|
| 结构切块 | 按 # / ## / ### 切分，保留 section_title、行号；跳过 paste-debug 类噪声 |
| 真 hybrid | BM25 + 向量双路，RRF 融合；禁止「向量有结果就忽略关键词」 |
| lexical rerank | 提升专名/标题排序 |
| 检索预算 | 每 Turn search_sources 默认 ≤3；重复 query 缓存并提示 |
| 结果瘦身 | excerpt 默认约 200 字；低分 hint 引导 read_file |
| 索引 | 上传异步索引+状态轮询；避免同步 embedding 冷启动拖垮上传 API |
| 降级 | hash embedding 等路径保证无重模型也能跑通契约 |
...
> 本节记录 **2026-07-13** 对照 `services/runtime`、`services/api`、`services/web` 与 Profile YAML 的核对结果。  
...
| 写作 Profile 工具表含 search_sources/delegate/export…，不含 run_command/grep | `writing.yaml` |
| Agent Profile 含 glob/grep/run_command/write_file… | `agent.yaml` |
...
2. **内部命令一览**：start-turn / cancel-turn / accept-patch / approve-tool 等请求响应；  
...
5. 检索默认 hybrid + lexical rerank；不要写成默认启用 cross-encoder。  
...
- **已修正关键不一致**：压缩顺序、apply_patch 路径、审批分级表述。  
