...
密钥入口 → 评测（`suite=ci` ≡ CI · `suite=golden` 切片）与只读观测（检索 / 信封 / Raw）。CI 证明门禁细节见 [28](proof-gate-and-ux-signals.md) · [`ci-proof-flow.png`](../assets/ops/ci-proof-flow.png)。
...
> **状态**：✅ 已落地（2026-07-24）— 事件 `audit` 三层 + Ops API/页；细项持续加厚见 [33](../archive/33-harness-maturity-backlog.md) HM5。  
> **定位**：Ops **观测**能力，与 §2 的 ci/golden **评测**并列；不是替代 `make gate`。  
> **流程图**：L1→L2→L3 读法见 [`06-ops-audit-l1-l2-l3.png`](../assets/rag/06-ops-audit-l1-l2-l3.png)（详解：[RAG-mental-model](../learn/mental/rag.md) 图册 #6）。
...
| 读法 | L1 常宽于 L3；`rank_method` 应为 `lexical`/`none`/`cross_encoder`。若见 `rank_method=hybrid` 且三层同构，曾是 two-level 未传播 ContextVar 的捕获兜底（已修，见 [33](../archive/33-harness-maturity-backlog.md) HM5） |
...
> **状态**：✅ 已落地 — 哈希必写 · **默认落全文**（`MODEL_ENVELOPE_SAMPLE_RATE` 默认 `1.0`）；高 fill / `MODEL_ENVELOPE_DEBUG` 仍强制全文。详见 [33](../archive/33-harness-maturity-backlog.md) HM4。
