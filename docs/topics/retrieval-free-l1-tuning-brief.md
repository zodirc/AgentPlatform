...
> **受众**：高级模型 / 下一轮调优负责人  
> **日期戳记**：2026-08-05（§14 批次 A 离线收束 · RET-4 **选型锁定 gte-small** · **未入库**）  
...
> **§14 批次 A 进度（2026-08-05）**：见 **§14.6**（离线收束 · gte 锁定 · 代码 `INDEX_VERSION=9` 已备 · **未重嵌 · 未入库**）  
> **§14.7 未执行排期 + 无 GPU 换模手册**：见 **§14.7**（明天他机 CPU 换 `thenlper/gte-small` 测试步骤）  
> **流程图**：检索 [retrieval-tuning-flowchart.png](retrieval-tuning-flowchart.png) · 上下文 [context-tuning-flowchart.png](context-tuning-flowchart.png)（`python3 scripts/gen_l1_tuning_lanes_zh.py` · 已标注 RET-4/CTX-8 本轮变更）
...
**RET-4 选型锁定（不可再开 L0 选型争论，除非影子 free 翻盘）**

冒烟 run `03569f22` · 每库 ≤20q · artifact `eval/reports/official/batch14/ret4_selection.json`
...
| 项 | 路径 / 说明 |
|----|-------------|
| CTX-8 文案 | `services/runtime/app/scenarios/agent/system.md` |
| RET-4 离线脚本与结果 | `scripts/official_bench/batch14_offline_selection.py` · `eval/reports/official/batch14/ret4_selection.json` |
| CTX-8 冒烟脚本 | `scripts/official_bench/run_ctx8_n2.sh`（DeepSeek `deepseek-v4-flash`；并行注意 inflight≤16） |
| Index bump + GPU embed | `vector_index.py` `INDEX_VERSION=9` · `embedder.py` CUDA auto |
| 流程图 | `docs/topics/*-tuning-flowchart.png`（本轮标注 RET-4/CTX-8） |

### 14.7 未执行排期（文档内待办）+ 无 GPU 换模测试手册

#### 14.7.1 未执行主线（严格按序 · 摘自 §14.4）
...
**禁止**：跳过 B/C 直接把 L0 +9pp 写入 SCORECARD；未重嵌只改查询模型；同批叠 RET-4 与 RET-11。
