...
> **受众**：下一轮调优负责人 / 高级模型  
> **日期戳记**：2026-08-05（**收敛重写** · CTX-8 N≥2 已收 · 停机线 **2/2** · RET-4 重嵌进行中 · **CTX-13 已收 · 组装层暴露面=0 → CTX-14/15/6 不开** · **残余细拆已收 · 上下文无高置信度下一刀** · **未入库**）  
...
> **当前平台锚（冒烟）**：检索 promote-off 均值 nDCG@10 **≈0.447**（`bcdbbb85`/`f92bc610`）；上下文 excl-infra F1 **≈0.41**（`fdd03298` 0.428 / `b5d24c9e` 0.411）  
> **停机线（EVAL-6）**：**2/2 已触发**（RET-9 + CTX-8）→ **停开新加法契约刀**  
> **下一刀序**：检索——嵌库就绪后 **RET-18 → REP-3 → RET-4 重嵌验收**；上下文——**工程停手**（组装层已否决；残余=尺子近对 / 定位 / 真错三分，见 §7.7；现行纪律下无高置信度抬分刀）  
...
| 3 | 温度计可信 | EVAL 配对 ✓；D-1 接受 writing-RAG | **REP-3 全量锚未入库** |
| 4 | 有停机线 | **2/2 已触发** | 遵守：停开加法契约 |

诚实带（对照非 KPI）：检索冒烟终态 **0.46–0.50**（须 RET-4/11）；上下文 F1 诚实带 **维持 ≈0.41**（组装层已否决；判分卫生/首读选窗等候选均非高置信抬分刀，见 §7.7；0.45–0.50 不再承诺）。narrativeqa≈0.25–0.29、scorer 近对类：**承认尺子/题型局限，不为刷分开刀、不换模背锅**。
...
- **不因检索侧阻塞（重嵌等待）强开低置信度上下文刀**；不立项「判分卫生 / 首读选窗 / 破停机线拧文案」为本轮抬分刀（§7.7 已评估不立项）。  
...
| 上下文 | excl-infra F1 **≈0.41** | CTX-9/EVAL-infra 后口径；勿与旧口径裸比 |
...
| wrong_answer 残余 | CTX-8 无正 Δ；CTX-13 组装暴露面=0；细拆=**尺子近对 / never_retrieved·定位 / 真推理错**（非「模型不行」单因） | **无高置信度下一刀**；不为刷分改判分；不停机线外拧契约；工程停手等检索 |
...
> 上下文：**CTX-13 已收**；组装层结构刀候选包按裁决**全部降级不开**；残余细拆已收 → **本轮无高置信度抬分刀，工程停手**。
...
- 上下文：**CTX-13 已收 · 暴露面=0 · 残余细拆已收** → 不承诺 F1 抬升；诚实带维持 ≈0.41；判分卫生/首读选窗等**不立项**；残余=尺子近对/定位/真错，**不是**「单模型不行」。  
...
- RET-18 / REP-3 / 观测刀（含已收 CTX-13）**不承诺分数**。
...
**当前唯一正确日历**：
检索——嵌库就绪 → **RET-18** → **REP-3** → **RET-4 验收** → 视余量 RET-11(b)；
上下文——**工程停手**（CTX-13 + 残余细拆已收；CTX-14/15/6 不开；无高置信度抬分刀）；
收束——PROD-1 首跑 + 终态四问书面作答。
