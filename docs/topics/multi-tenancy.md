...
> **另票：** `search_records` 真表见 [`17`](search-records.md) RE5（与已否决的 Org/share 无关）。  
> **定位：** 把单机自用栈升级为**更大的多人 Web 平台**时的划分结构——「谁登录、打开哪片世界、别人能不能看见」；迈向成熟 Agent 的关键一步。**不是** SaaS 清单打勾，也**不是**用多容器/多线程本身冒充租户隔离。  
> **硬约束：** [`13` R1–R5](../README.md)；不改 `AgentEngine` while；不改用户可见交互哲学（流式 / 取消 / Plan 同意门 / diff-first）。  
> **前置已落地：** [`16`](user-session-history.md) `owner_user_id` · [`23`](writing/work-model.md) Work-over-Session · [`15` §6](../README.md) IX5/RE4 · [`17`](search-records.md) 多表 ACL 槽位  
> **关联：** [`02`](../core/02-architecture.md) · [`03`](../core/03-docker-runtime.md)（含多副本 §8.5 / `make up-ha`）· [`07`](../core/07-domain-model.md) · [`12`](../core/12-model-harness.md) Intake/Guard
...
```text
已落地：  Principal → 默认 Work → work_root
              └── Session*（对话线程，可换）
                    └── Turn/Run（仍 1:1；loop 不变）

（迁移前遗留心智）单机：部署级 /workspace ≡ 全世界 —— 现由「认领为默认 Work」兼容
```

**成熟度自检（当前态）：** 隔离结构已落地；执行面零感；**划分默认开启**；每个登录账号都是普通 Principal（含部署者本人）；**一人一默认世界**（无 Org / 无作品分享）。

| 支撑级别 | 含义 | 现状 |
|----------|------|------|
| **L0 结构** | Principal → 默认 Work → Session.`work_id` → work_root | ✅ |
| **L1 执行隔离** | Intake 冻结 TenantContext；路径沙箱；检索 `seed OR work_id` deny 可证 | ✅ |
| **L2 记忆/索引同界** | 记忆按 Work 分仓；索引按 Work 分区同步 | ✅ |
| **L3 并发软闸** | 单 runtime 活跃 Turn 上限；超额拒绝 | ✅ |
| **L4 多副本 HA** | 双 runtime + 亲和（`make up-ha`）；按并发 Turn 扩容 | ✅ 可选 |
| **L5 Org / share / 切换器** | 团队共享盘、显式分享、多 Work UI | ❌ 否决 |
| **L6 字段加密 / 一用户一容器** | 合规硬隔离 | ❌ 非目标 |

速率口径见 **§8**：热路径增量预算约 **1～5ms**（主键/缓存解析）；扩容单位是 **并发活跃 Turn**，不是登录人数。
...
| **扩容面（[`03` §8.5](../README.md)）** | 同时跑多少 Turn？挂了谁接手？ | api/runtime 多副本、worker、Turn 亲和、共享卷 | 划对了也会挤在单 runtime 上变慢 |
...
4. **扩容时多租户的约束**（与 [`03` §8.5](../README.md) 对齐）：共享 `/data`（或对象存储）上按 `works/{work_id}` 分区；`runs.runner_id` + 命令路由；同 Work 写冲突策略；**禁止**把隔离赌在「请求碰巧打到同一进程的内存」。  
   **产品口径：** 仅有划分、单 runtime 也能「不串味」，但多人同时跑 Turn 会挤——**多用户 Web 平台应使用 `make up-ha`（MT7）**，否则吞吐上像空架子。
...
| 对象 | 是否跟账号 | 说明 |
|------|------------|------|
| 登录会话（cookie / token） | ✅ | 已有 [`16`](user-session-history.md) |
...
1. **Session 不当作品容器**（[`23`](writing/work-model.md) / ADR-020）  
2. **RAG 热路径不加裁判模型**（[`15`](rag-and-sources.md)；否决 LLM-ACL）

多租户是同一刀的**第三面**：把「单机自用默认全世界可读」升级为「主体有私有世界」，否则：

- 多用户同栈时资料/书稿/记忆串味 → **交付诚实性与信任崩盘**（比慢更致命）  
- 评测与线上无法证明「deny 成立」→ **Proof 面缺口**  
- 后续 `search_records`、Skills 会被迫在错误边界上打补丁  
...
> 任何实施方案若触碰下列红线，直接否决——与 [`26`](writing/plan-suggest.md) 同级。
...
| Plan 同意门 / `plan_phase` 硬闸 | [`25`](writing/runway.md) 不动 |
...
- 查询路径建库 / per-session 索引（已否，见 [`15`](rag-and-sources.md)）  
- **LLM-ACL**（「模型判断能不能看」）  
...
| **Session** | transcript、审批、压缩窗、**绑定的 work_id** | 章节目录树（已由 [`23`](writing/work-model.md) 否定） |

**铁律：**

1. **隔离键 ≠ `session_id`**（[`15` §6](../README.md) 已写；此处升级为宪法）  
...
| 场景 | 结构上发生什么 | 用户感到什么 |
|------|----------------|--------------|
| **任意账号（含你）** | 确保默认 Work → Session 绑定 `work_id` → Turn 带 TenantContext | 登录、聊天、写书；单默认 Work，无额外步骤 |
...
开闸：真实多用户不可互看；deny golden；**单人默认 Work** 交互与今日同型。身份复用 [16](user-session-history.md) 的 `owner_user_id`；作品根见 [23](writing/work-model.md) / [27](multi-tenancy.md)。
...
### 5.4 `search_records`（[`17`](search-records.md)）
...
- **产品默认：** 一账号一默认 Work；**多 Session** 换聊天线，不换世界（对齐主流 Agent）  
- **不做** 同账号多作品切换 UI / Org 协作（§11）  
...
**不做** `POST /works/{id}/shares` / Org 成员共享（§11）。

会话历史 [`16`](user-session-history.md) 过滤规则扩展为：`owner_user_id` **且**（可选）`work_id` 范围；不把全文塞进历史抽屉。
...
| 默认 | 理由 |
|------|------|
| **默认 deny** 他人 private | 成熟产品默认；**不做**显式 share / Org |
...
> SLO 仍以 [`10`](../core/10-product-experience.md) 为准：**不因本方案放宽**。
...
| 4 | **会话列表** [`16`](user-session-history.md) | `WHERE owner_id=` | 可选再 `AND work_id=`（有索引则同量级） | **无感** | 离 Turn 热路径 |
...
| 16 | **切 Session** | 换 id + 载 transcript | 通常仍绑同一默认 Work | **无感** | Turn 外 |
| 17 | **登出 / 再登录** | 清 cookie | 无额外租户握手 | **无感** | — |
...
3. 今日实现：`POST /start-turn` → `BackgroundTasks` 进**同一** asyncio 进程（单 uvicorn worker 典型形态）；**已有**可观测并发软闸 `RUNTIME_MAX_INFLIGHT_TURNS`（默认 16，MT5b）；超额 fail Turn。多副本用 `make up-ha`（MT7）。
...
| **runtime** | **1（默认）** | **仍 1，直到并发 Turn 顶 SLO** | 并发顶 SLO 时：`make up-ha`（`runner_id` + `RUNTIME_URL_MAP`，[`03` §8.5](../README.md)） |
...
| session transcript / events | ∝ Turn 数 | 已有滚动/压缩；按 owner 可删会话（[`16`](user-session-history.md)） |
| `.agent/work/history` | ∝ 草稿快照 | 已有 keep 上限（[`23`](writing/work-model.md)） |
...
| **投影** | `TurnView` ≡ 事件终态 | 异步投影；以事件为真相（[`08`](../core/08-event-projection-pipeline.md)） |
...
| **检索索引** | 与磁盘最终一致 | 异步 sync；允许短暂 `index_lag`；查询不建库（[`15`](rag-and-sources.md)） |
...
未做多副本前，**不要**对外承诺「百人同时写作都跟手」——应承诺「百人划分不串味；同时活跃 Turn 按副本与配额扩展」。并发闸（MT5b）与 HA（MT7）已具备；部署多人并发请用 `make up-ha`。
...
|----|------|------|
| **MT0** | 本文 + ADR-021；交叉引用 | ✅ |
| **MT1** | `works` + 自动默认 Work + Session.`work_id` + StartTurn TenantContext 字段 | ✅ |
| **MT2** | runtime `work_root` 路径沙箱；approve/deny 重绑 Context；seed 只读可见 | ✅ |
| **MT3** | 检索命中租户过滤；索引 `work_id`/`visibility`；scheduler 按 Work 同步 | ✅ |
| **MT4** | `remember`/`recall` 按 `work_id` 分文件命名空间 | ✅ |
...
5. **写作 / RAG / 记忆 / 委派 全部挂到同一 Work 根；概念复用 [`23`](writing/work-model.md)，不平行发明。**  
...
| 非目标 | 说明 |
|--------|------|
| Org / 作品显式 share / 团队共享盘 | **否决（原 MT6）**；多人 = 各开各的默认 Work，不是协作 ACL |
| 同账号多作品切换 UI | 不做；一账号一默认世界 + 多 Session（对齐主流 Agent） |
| K8s 命名空间级硬隔离 / 每租户一集群 | 部署拓扑另案；本方案先**同栈逻辑多租户**（见 §0.1） |
| 用「加副本 / 加线程」代替 ACL | 扩容面解决吞吐，不解决「谁能看见」 |
| 公网 SSO 全家桶 / SCIM | 身份可后接；先复用 [`16`](user-session-history.md) |
| 字段级加密 / BYOK | Guard 内容防护保留；合规套件后置 |
| 同 Turn 多 Work 联邦检索 | 复杂度爆炸；本产品不切多世界 |
| 租户市场 / MCP 插件商店 | 平台非目标（见 [`01`](../core/01-problems-and-goals.md)） |
...
1. ~~**个人默认 Work 何时自动创建？**~~ → **已定：** 注册 / 登录 / 建 Session 时 `ensure_default_work`（不挡 TTFB）。  
2. **seed 语料计费与配额是否计入租户？** 倾向不计入；仅 private 摄入计数。  
3. ~~**单测夹具 per-work？**~~ → **已有** deny 单测 + `shared.17` / `--filter tenant`；夹具继续按隔离扩。
...
| 评分弱项 | 本文如何结构性地还债 |
|----------|----------------------|
| 企业就绪 4.5 | **个人多用户划分已还债**；Org/协作 ACL 明确不做，避免假 SaaS |
| RAG 规模 7.0 | IX5 隔离键与谓词已落地，多用户可证明 deny；万档运维仍后置 |
| 写作路径与 Session 解耦 | 落实 [`23` §11](../README.md)，Session 继续不管书；一账号默认一世界 |
