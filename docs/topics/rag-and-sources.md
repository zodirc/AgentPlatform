...
> **关联**：文风/成稿走廊 → [14](writing/quality.md)；工具宪法 → [06](../core/06-tools-and-context.md)；速率 → [13](../core/13-rate-redlines.md) R1–R5；部署默认栈 → [03](../core/03-docker-runtime.md)。
...
| ACL / 私有库 | **✅ 个人默认（MT5c）**；无 Org/share | RE4 / IX5 · [27](multi-tenancy.md) |
| `search_records` 真表 | **产品开闸** | RE5 / [17](search-records.md) |
...
> **纪律**：可观测真源是 **事件 / 契约**；**主查看面是 Ops 内部观测台**（只读看前台用户 Turn），不是工作台侧栏。只加前台 `RetrievalView` = **假可观测**。详案：**[33](../archive/33-harness-maturity-backlog.md) HM5** · 承载面 **[29](ops-eval-console.md) §6**。
...
服从 [13](../core/13-rate-redlines.md)：轻量字段可随工具返回；重体积异步；不挡首 token；Ops **只读**。

### 3.4 向量相似度原理（余弦 · 欧氏 · 归一化）

> 面试常挖「为何用余弦」「和欧氏啥区别」。本节用**二维小数**把账算清楚；口述版见 [21](../learn/agent-system-qa.md) Q13。

#### 先建立直觉（少符号）

把每段文字想成平面上的一根箭（真实系统是几百维，算法一样）：

| 名字 | 在比什么 | 怎么读结果 |
|------|----------|------------|
| **余弦相似度** | 两根箭的夹角：指得越同向越高分 | **越大越像**（满分约 1，反向约 -1） |
| **点积** | 对应分量相乘再相加 | **越大越像**；箭更长时分数容易虚高 |
| **欧氏距离** | 两个箭头尖端之间的直线距离 | **越小越像** |

RAG 要的是「意思像不像」≈「方向像不像」→ 默认讲余弦；库里若已把箭都收成「长度 1」，实现上常用点积（更快），和余弦排出来的名次一样。

#### 三个量怎么算（白话公式）

约定：查询向量叫 **Q**，某篇文档向量叫 **D**。二维时 Q=(Qx, Qy)，D=(Dx, Dy)。
...
二维够手算；生产几百维只是把「每一维都乘加一遍」，形式不变。

#### 例子 A：同向、垂直、反向

查询固定为 **Q = (1, 0)**（水平向右，像「猫」这个方向）。

| 文档 | 向量 | 含义 | 点积 | 长度(D) | 余弦怎么来 | 余弦 | 欧氏 |
|------|------|------|------|---------|------------|------|------|
| A | (2, 0) | 同向、更长 | 2 | 2 | 2 / (1×2) | **1.0** | 1.0 |
| B | (1, 0) | 同向、一样长 | 1 | 1 | 1 / (1×1) | **1.0** | **0** |
| C | (0, 1) | 垂直向上 | 0 | 1 | 0 / (1×1) | **0** | ≈1.41 |
| D | (-1, 0) | 反向 | -1 | 1 | -1 / (1×1) | **-1.0** | 2.0 |

读法：

- A 和 B **余弦都是 1**：方向完全一样；A 更「长」不影响余弦。  
- 若只看 **点积**：A=2 > B=1 → 会误以为 A 更像（其实只是更长）。  
- 若只看 **欧氏**：B 距离 0 最近，A 反而更远——欧氏比的是落点，不是语义方向。  
- C 垂直 → 余弦 0；D 反向 → 余弦 -1。

#### 例子 B：忘了归一化时，欧氏会害「长而相关」的文档

仍用 **Q = (1, 0)**。下面三个候选：

| 标签 | 向量 | 和 Q 是否同向 | 余弦（约） | 欧氏（约） |
|------|------|---------------|------------|------------|
| 短·相关 | (0.8, 0.1) | 几乎同向 | **0.99** | **0.22** |
| 长·相关 | (8, 1) | 与上一行同一方向（刚好 ×10） | **0.99** | **7.1** |
| 短·不太相关 | (0.2, 0.3) | 偏得更多 | 0.55 | 0.86 |

**按余弦排序（越大越好）：** 短·相关 ≈ 长·相关 → 再才是不太相关。合理。  

**按欧氏排序（越小越好）：** 短·相关(0.22) → 不太相关(0.86) → **长·相关(7.1)垫底**。  

长·相关明明方向对，只因为箭太长、落点离 Q 远，就被欧氏判成最差。  
这就是「向量没收成单位长、却裸用欧氏」时的典型事故（长文本 / 范数偏大时更常见）。

手算提示（短·相关）：

```text
点积 = 1*0.8 + 0*0.1 = 0.8
长度(D) = sqrt(0.8^2 + 0.1^2) ≈ 0.806
余弦 = 0.8 / (1 * 0.806) ≈ 0.99
欧氏 = sqrt( (1-0.8)^2 + (0-0.1)^2 ) ≈ 0.22
```

长·相关是短·相关每维乘 10，夹角不变 → 余弦仍约 0.99；但欧氏大约变成 7.1。

#### 例子 C：先把每根箭收成「长度 = 1」，三种排法名次一样
...
```text
余弦 = 点积          （分母长度都是 1，除掉等于没除）
欧氏^2 = 2 - 2×余弦  （余弦越大，欧氏越小）
```

所以：**余弦从高到低 = 点积从高到低 = 欧氏从低到高**，名次相同。  

把例子 B 的三个向量都先单位化再比，不会再出现「长·相关垫底」。  

工程含义：入库前先归一化，检索用 **点积** 即可（少一次除法），对外仍可说「按余弦检索」。  
「库已经归一化了，所以余弦比欧氏更准」——是假对比，名次本来就一样。

#### 那为什么还说「业界选余弦、不选欧氏」？

1. **目标对**：我们要方向像不像，余弦直接说这个。  
2. **更耐造**：某条链路忘了归一化时，余弦仍主要看方向；裸欧氏会重演例子 B。  
3. **实现习惯**：向量库常提供 cosine / inner product；归一化后用点积最快。  

不是欧氏公式错了，也不是归一化后余弦「算得更准」。
...
|------|--------|------------------|
| 余弦 / 点积 | 向量通道内部排序 | — |
| BM25 | 词法通道内部排序 | **不能**（量级完全不同） |
| RRF | 看两路「排第几」再融合 | 正确做法 |
| 词法/交叉编码重排 | 融合后再微调顺序 | 另算 |

假融合：A 余弦 0.82、BM25 12.3；B 余弦 0.79、BM25 3.1。  
若算 `0.5*余弦 + 0.5*BM25`，BM25 会把结果吞掉——没有物理意义。应看两路名次做 RRF。
...
- **HNSW 建边时的「更近」**：同一余弦距离（`vector_cosine_ops` / `<=>`）；距离小才进入邻居候选并连边——白话步骤见 [RAG-mental-model §5.0b](../learn/mental/rag.md#50b-hnsw什么叫更近建边时怎么比)。
...
| RE5 | `search_records` 真表 | ⏳ 见 [17](search-records.md) |
...
| **RO1** | 检索可观测 → **Ops 观测台看前台用户 Turn**（三层审计） | ✅ **已落地**（`retrieval.completed.audit` + `GET /ops/retrieval/turns/{id}` + Ops「检索审计」页）· 详 [33](../archive/33-harness-maturity-backlog.md) HM5 · [29](ops-eval-console.md) §6 |
...
**结构正文（Tenant / Work / 速率宪法 / 分期）：** [`27-multi-tenancy`](../README.md) · [ADR-021](multi-tenancy.md)。本节只保留检索面要点。
...
开闸（已落地 MT5c）：SQL `visibility=seed OR work_id=$current`；跨 Work deny 单测 + `shared.17` / `--filter tenant`；**单人默认 Work** 交互与今日同型。**不做** Org/显式 share。身份复用 [16](user-session-history.md) 的 `owner_user_id`；作品根见 [23](writing/work-model.md) / [27](multi-tenancy.md)。
...
| 速率 | 搜索热路径无 sync/重嵌；无默认同步 LLM；首 token / 工具墙钟不触 [13](../core/13-rate-redlines.md) 红线 |
