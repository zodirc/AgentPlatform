...
| **S3** | 扩库与深化 | A10 A11 A4 A5 A13 A17–A21 | 千～万档资料；质量分离线 | **代码已落地（2026-07）**；合入前跑 runtime-test / eval |
...
**说明**：原「门铃」项已按用户指示全部落地；仍遵守速率红线（用户触发 / 离线 / 超时降级）。

| ID | 落地摘要 | Commit 主题（约） |
|----|----------|------------------|
| A10 | `RETRIEVAL_BACKEND=pgvector` + HNSW；默认仍 json | Add optional pgvector ANN backend… |
| A11 | doc↔chunk 并行召回 + 超时降级 chunk-only | Add parallel doc-to-chunk recall… |
| A4 | `/verify` + `verify.sample` ≤5% | Add user-triggered /verify… |
| A5 | `make eval-rubric` 离线启发式 / ≤5% | Add offline rubric judge… |
| A13 | `remember` / `recall` 分仓按需工具 | Add on-demand remember and recall… |
| A17 | `COMPACT_MODEL_*` 小模型分流 | Route context compact through… |
| A18 | 打字 debounce → warmup-retrieval | Warm retrieval during composer… |
| A19 | 阶段化 ToolScope（交付后 / 末段缩工具） | Shrink tool schemas in late… |
| A20 | 蓝图 `docs/18` + `search_records` stub | Add search_records stub… |
| A21 | 提示词 + 事实核查按钮 + `critique.batch` | Add on-demand critique… |

### S3 冲刺出口

- [x] A10–A21 最小可合并切片已提交（2026-07）
- [ ] `make runtime-test` / `make eval-all` / `make eval-retrieval` 合入前全绿
- [ ] 生产启用 pgvector 时：换镜像 / 扩展 + `RETRIEVAL_BACKEND=pgvector` 并 rebuild index
...
```text
S0
  [x] T-A1  ToolExecutor jsonschema gate + unit tests
  [x] T-A2  Turn evidence set + unverified citations + golden
  [x] T-A3  Misuse counters / eval report fields
  [x] T-A14 Model egress allowlist

S1
  [x] T-A6  agent(/writing) system + Intake plan hint
  [x] T-A7  open_items backfill on turn terminal
  [x] T-A8  delegate context_refs + hot_files inject
...
S3
  [x] Epic-RAG-scale   A10/A11 after A9
  [x] Epic-offline-qa  A4/A5/A21
  [x] Epic-memory      A13
  [x] Epic-speed-tune  A17/A18/A19
  [x] Epic-multitable  A20 design-first + stub
```
