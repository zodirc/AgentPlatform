...
- **默认镜像**：`Dockerfile.retrieval` — 构建期烘焙 `EMBEDDING_MODEL` 至 `/app/models-baked`；entrypoint 首次启动同步至 `/data/models`；默认 `EMBEDDING_BACKEND=sentence_transformers`；`RETRIEVAL_BACKEND=pgvector`；向量索引 `INDEX_VERSION=7`（叶预算 + path/tag embed + BM25/RRF profile；见 [`15` §9](../README.md)）
