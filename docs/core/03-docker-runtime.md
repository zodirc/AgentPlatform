...
容器拓扑是物理部署视角。逻辑上仍必须遵守 [`docs/02-architecture.md`](02-architecture.md) 中定义的五层模型：
...
权威决策：[ADR-019](03-docker-runtime.md)。
...
- **默认镜像**：`Dockerfile.retrieval` — 构建期烘焙 `EMBEDDING_MODEL` 至 `/app/models-baked`；entrypoint 首次启动同步至 `/data/models`；默认 `EMBEDDING_BACKEND=sentence_transformers`；`RETRIEVAL_BACKEND=pgvector`；向量索引 `INDEX_VERSION=8`（叶预算 + path/tag embed + BM25/RRF profile；见 [`15` §9](../README.md)）
...
权威决策：[ADR-018](03-docker-runtime.md)。
...
> **口径（2026-07-24）**：文件类工具走 `_resolve_path`。exec 经 `run_shell_command` / `run_argv_command`；镜像含 **bubblewrap** 时默认 OS 沙箱包裹（docs/31 · SB1）——**无产品级 env 旋钮**。详见 **[31](../topics/sandbox.md)**。
...
- `run_command` / `read_lints`：有 bwrap 时 **FS 仅 work_root 可写**（护主机/护兄弟 Work）；**默认保留出网**（用户批准的 `curl https://…` 可用）；子进程 env deny-by-default（固定允许集）
