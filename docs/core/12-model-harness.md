...
> **状态**：部分落地（2026-07-13）— **AH1 ✅**；**AH2–AH4 / AH-obs 已落地核心路径**。**§5.1**：① WT5/cache 布局经 AQ1/WN3/**HM6** ✅ ② 变短≈压缩经 **HM1/HM3** ✅ ③ Proof 延迟盯梢持续。详见 [33](../archive/33-harness-maturity-backlog.md)。  
> **范围**：**广义 Agent Harness**——包住 frontier model、决定「好不好用」的整层工程：Intake · Context · Tools 执行纪律 · Model · Cancel/超时/watchdog · Eval/SLO 与可观测。  
> **不在范围**：改写 `AgentEngine` while 语义；恢复固定 pipeline；K8s / MCP / 多租户模型市场；**写作去 AI 腔**（走 writing `system.md`，见 [`14`](../topics/writing/quality.md)，不塞进 Harness 引擎）。

关联：[`05-agent-runtime.md`](05-agent-runtime.md)（loop 细则）· [`06-tools-and-context.md`](06-tools-and-context.md)（工具与 Context 契约）· [`20`](../learn/context-compaction-walkthrough.md)（压缩演练）· [`24` §4.6](../README.md)（WT5）· [`33`](../archive/33-harness-maturity-backlog.md)（**HM 工程完善票**：预压缩 / raw 审计 / 增量摘要 Proof）· [ADR-014](05-agent-runtime.md)（Intake）· [ADR-015](05-agent-runtime.md)（Cancel）· [ADR-016](05-agent-runtime.md)（超时/watchdog）· [ADR-019](03-docker-runtime.md)（供应商配置）· [`10-product-experience.md`](10-product-experience.md)（体验 SLO）· [`11-eval-and-golden-turns.md`](11-eval-and-golden-turns.md)（延迟门禁）。
...
> 「厚」不能牺牲「快」：每一项能力增强都要通过热路径成本审视，不得回归 [`11` §1](../README.md) 的 TTFB / 首 token / Cancel SLO。
...
### 4.1 继承 SLO（权威定义见 [`11` §1](../README.md)）
...
5. **可测才算数**：性能改动挂 [`12`](12-model-harness.md) 延迟门禁。
...
与 [`06` §6.1](../README.md) 对齐：`ContextEnvelope` 为 assemble 权威输出；`system_blocks` **不**回写 `TurnState.messages`。

### 5.1 下一刀优化思路（2026-07-21）

> 三条线**分开推进**，都服从 [`13`](13-rate-redlines.md) R1–R5；**不改** `AgentEngine` while / 发送路径 / Plan 同意门。  
> **不混入**：写作去 AI 腔 → writing [`system.md` Prose defaults](../services/runtime/app/scenarios/writing/system.md) / [`14`](../topics/writing/quality.md)；资料库 RAG → [`15`](../topics/rag-and-sources.md)（下一刀 RQ1 见 [`15` §9](../README.md)）。

排期建议：`① cache 布局 WT5` → `② 变短≈压缩` → `③ Proof 延迟盯梢`。

#### 5.1.1 Cache 布局（WT5 · Model × writing 组装）

**问题：** 写作常把 `system.md` + cards + work index + focus/prev **焊成一整段 system** 再打 `cache_control`。换章/换话 → **整段 miss**，稳定正文陪葬。仪表盘命中率 ~10–15% 多半正常（messages / tool_results 本就不会进稳定前缀）。细则权威：[`24` §4.6](../README.md)。
...
**产品体感（对齐 Cursor / Claude Code 自用观察）：** 平时上下文**一路涨或持平**；**明显变短**应对得上自动压缩链或用户 `/compact`。单条进窗 `budget` 截断是「这条没那么肥」，不是整段对话暗中变短。机制教学见 [`20`](../learn/context-compaction-walkthrough.md)。
...
**已有：** SLO（[`10` §1](../README.md)）TTFB≤300ms / 首 token / Cancel≤500ms；事件 `context.reported`（`assemble_ms`）、`usage.reported`（retry/cache）；golden `latency.*`（[`11`](11-eval-and-golden-turns.md)）。
...
北极星服从 [`11`](11-eval-and-golden-turns.md)：愿意连续自用数周，不因模型层不可预期或不跟手而弃用。
