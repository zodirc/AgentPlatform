...
Phase 1 起交付；用于 Web 设置页，**无需重启 runtime** 即可切换供应商。权威语义：[ADR-019](03-docker-runtime.md)。
...
> **无独立 `ResumeTurn`**：审批 interrupt 后的恢复由 `ApproveToolCall` 完成（见 [ADR-009](contracts.md)）。
...
- **语义权威**：[ADR-015](05-agent-runtime.md)
...
**完整目录**（含 Phase 标注）：[ADR-004](08-event-projection-pipeline.md) · 机器枚举：`packages/contracts/schemas/events/types.json`
