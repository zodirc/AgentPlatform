...
api 受理 `CancelTurn` 时，除转发 `cancel-turn` 命令外，须 **写入** `runs.cancel_requested_at`（及 `cancel_force`）。runtime 在执行全过程轮询该标志（与内存 `TurnState.cancelled` 并行，取先生效者）。详见 [ADR-015](05-agent-runtime.md)。
...
- [ADR-002](03-docker-runtime.md) — 存储
- [ADR-005](05-agent-runtime.md) — 循环内核
- [ADR-009](contracts.md) — 四层协议
- [ADR-011](07-domain-model.md) — Run 与 Turn 1:1 决策
- [ADR-013](09-product-modes.md) — `scenario_id` 与 Profile
- [ADR-015](05-agent-runtime.md) — Cancel / interrupt / resume
- [ADR-016](05-agent-runtime.md) — 超时与卡住检测
- [ADR-017](contracts.md) — 边界校验与 payload schema
- [ADR-019](03-docker-runtime.md) — 模型供应商 DB 热配置
