...
旧项目的 `event_classification_node` **不恢复**。任务理解拆为三层（[ADR-014](05-agent-runtime.md)）：
...
loop 形态需要 Step 粒度的事件，而不是粗糙的阶段事件。**权威事件类型与 envelope** 见 [ADR-004](08-event-projection-pipeline.md)。
...
**Cancel / interrupt / resume 权威语义**：[ADR-015](05-agent-runtime.md)。
...
runs.status IN (running, interrupted)
  AND 最新 turn_events.ts 早于 now() - 180s
...
- **`collab`（目标态 · 未实施）**：编排者姿态 + agent 工具面 + `researcher` 等；见 [`37-collab-multi-agent.md`](../topics/collab-multi-agent.md)
...
见 **[`appendix-migration.md`](../archive/appendix-migration.md)**（单一维护点）。
