...
详见 [ADR-013](09-product-modes.md)。
...
**目标态第三入口**：`collab`（多 Agent 协作）——取消薄 stub `interview`；规范见 **[37-collab-multi-agent.md](../topics/collab-multi-agent.md)**（设计定稿 · 代码未实施）。
...
- 跨 Session 续写同一作品：读 `sections/` 或 `drafts/`（见 [23](../topics/writing/work-model.md)）
...
| 软/硬取消 | `POST /api/v1/turns/{id}/cancel`；`force` 语义见 [`contracts.md`](contracts.md) §2.1、[ADR-015](05-agent-runtime.md) |
...
新增 / 替换 scenario（目标例：`collab` 替 `interview`，见 [37](../topics/collab-multi-agent.md)）时 **只允许**：
...
- [ADR-005](05-agent-runtime.md) — 单 loop  
- [ADR-006](06-tools-and-context.md) — 能力即工具  
- [ADR-007](05-agent-runtime.md) — 子 agent  
- [ADR-013](09-product-modes.md) — Scenario 与 Profile 扩展  
- [ADR-018](03-docker-runtime.md) — Web 前端栈与 `scenarios/*` 布局
- [ADR-019](03-docker-runtime.md) — 模型供应商 Web 管理 + DB 热生效
