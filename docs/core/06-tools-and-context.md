...
本文件遵循与 [`docs/02-architecture.md`](02-architecture.md) 和 [`docs/05-agent-runtime.md`](05-agent-runtime.md) 相同的原则：
...
**验收**：[`11-eval-and-golden-turns.md`](11-eval-and-golden-turns.md) §5.2 为 **Phase 1b 阻断项**——无 golden 覆盖不得进入 Phase 2。
...
| `rename_file` | core | 重命名/移动已有文件 | 非导出；禁 seed；默认不覆写 |
...
`project_context` / `runtime_context` 与 envelope 分区的**补齐优先级**见 Agent Harness 总纲 [`12-model-harness.md`](12-model-harness.md)（AH3a）；实现前以本文类型契约为准，状态以 `14` 文首为准。
...
原始 messages 另有 append-only `session_raw_snapshots`（HM2），**永不**直接当模型窗。详见 [`20`](../learn/context-compaction-walkthrough.md) · [`33`](../archive/33-harness-maturity-backlog.md)。
...
> 逐步教学模拟见 [`20-context-compaction-walkthrough.md`](../learn/context-compaction-walkthrough.md)。
...
检索质量下一刀（path+tag embed、树叶预算、库级 hybrid profile；**不开默认 CE**）见 [`15` §9](../README.md)。
...
平台通过 **ScenarioProfile** 为每个 Turn 裁剪工具与子 agent（见 [`09-product-modes.md`](09-product-modes.md)）。工具**实现**均在 `tools/core/`；场景只 **登记** 工具名。
...
为统一 [`docs/03-docker-runtime.md`](03-docker-runtime.md) 的依赖约束与本文件的主路径要求，Phase 1b 的检索最小能力定义如下：
...
| `read_file` | read | ✓ | ✓ | |
| `list_dir` | read | ✓ | ✓ | |
| `rename_file` | write* | ✓ | ✓ | 窄改名/移动；默认无审批；非内容重写 |
...
见 **[`appendix-migration.md`](../archive/appendix-migration.md)**。
